...
function resolveWorkingDir(dir?: string): string | undefined {
  return dir ? path.resolve(/*turbopackIgnore: true*/ process.cwd(), dir) : undefined;
}
...
const run = async (cmd: string, cwd: string | undefined = undefined, timeout = 30_000) => {
  const { stdout, stderr } = await execAsync(cmd, { cwd, timeout });
  return (stdout + (stderr ? `\nSTDERR: ${stderr}` : '')).trim();
};

const safe = async (cmd: string, cwd: string | undefined = undefined, timeout = 30_000): Promise<string> => {
  try {
...
export const gitStatus    = (dir?: string) => safe('git status --short --branch', resolveWorkingDir(dir));
...
export const gitPull      = (args = '')    => safe(`git pull ${args}`, undefined, 60_000);
export const gitPush      = (args = '')    => safe(`git push ${args}`, undefined, 60_000);
export const gitBranch    = (args = '')    => safe(`git branch ${args}`);
export const gitCheckout  = (b: string)    => safe(`git checkout ${quoteArg(b)}`);
export const gitStash     = (args = '')    => safe(`git stash ${args}`);
export const gitReset     = (args: string) => safe(`git reset ${args}`);
export const gitShow      = (ref = 'HEAD') => safe(`git show ${quoteArg(ref)} --stat`);

export async function gitClone(url: string, dest?: string): Promise<string> {
  if (!url.startsWith('http') && !url.startsWith('git@'))
    return 'Invalid git URL.';
  const cmd = dest ? `git clone ${quoteArg(url)} ${quoteArg(dest)}` : `git clone ${quoteArg(url)}`;
  return safe(cmd, undefined, 120_000);
}

export async function gitInit(dir?: string): Promise<string> {
  const target = resolveWorkingDir(dir);
...
export async function gitBlame(filepath: string): Promise<string> {
  ...
  return safe(`git blame ${quoteArg(filepath)} --line-porcelain`);
}
